...
<?php
    if (isset($_POST['merge'])){
        $from_string = $_POST['tag_from'];
        
        $pattern1 = '/, *,/';
        $pattern2 = '/, *$/';
        
        $from_string = preg_replace($pattern1, '', $from_string);
        $from_string = preg_replace($pattern2, '', $from_string);
       
        $from = explode(',', $from_string);
        
        
        $to_string = $_POST['tag_to'];
        
        $pattern1 = '/, *,/';
        $pattern2 = '/, *$/';
        
        $to_string = preg_replace($pattern1, '', $to_string);
        $to_string = preg_replace($pattern2, '', $to_string);
       
        $to = explode(',', $to_string,1); //Array is used, but we need only one element
        ...
        foreach($from as $from_name){
            ...
            foreach($tagmaps as $map){
                ...
                //If we have TAG1, TAG2 and 
                //want to change TAG1 to TAG2 
                //(To exclude a situation when there will be TAG2, TAG2)
                //So we just skip it
        ...
        $_SESSION['msg'] = 'Теги успешно объединены';
    }
    ...
    <div id="tabs-2">        
        ...
        <form name="tags_merge" action="/admin/page/tags/" enctype="multipart/form-data" method="Post">
            <fieldset>
                ...
                <input class="text-long" id="tag_from" name="tag_from">
                </input>
                </p>
                <p>
                <label>Заменить на:</label>
                <input class="text-medium" id="tag_to" name="tag_to">
                </input>
                </p>
