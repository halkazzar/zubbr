<?php
    ...
    if(isset($_GET['tag'])){
        $question_mode = 'tagged';
        $tag_title = $_GET['tag'];
        
        $tag = Tag::find_by_exact_name(trim($tag_title));
        if(!empty($tag)){
            $total_questions = Question::count_by_tag($tag->tag_id); 
        }
        else{
            $total_questions = 0;
        }
        $page_title = '}:) - Образование в Казахстане и за рубежом '.$pipe.' Вопрос/Ответ ' .$pipe. ' ' . $tag_title;
        $meta_description = 'Вопросы по тегу ' . $tag_title;
        
        
    }
    elseif (isset($_GET['cat_id']) && isset($_GET['object_id'])){       
                ...
                <?php
                    ...
                    <div class="post-meta archive-pagination">
                        <?php    if($pagination->has_previous_page()) { 
                                ...
                                elseif (isset($tag_title)) echo "/tagged/{$tag_title}";
                                echo "/page_";
